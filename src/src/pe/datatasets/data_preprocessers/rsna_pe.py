...
class RSNAPePreprocessing(DataPreprocessing):
    ...
    def read_splits_(
        self, split: str = "train", engine: Optional[str] = "pyarrow"
    ) -> pd.DataFrame:
        """
        ...
        """
        df = pd.read_csv(
            os.path.join(self.dataset_path, split + ".csv"), engine="pyarrow"
        )
        return df

    @staticmethod
    def get_unique_studies_(
        df: pd.DataFrame, studies_colunm: str = "StudyInstanceUID"
    ) -> list:
        """
    ...
    def load_dicom_(path: str) -> DicomItem:
        ...
        dicom_object = DicomItem(
            array=dicom.pixel_array,
            index=dicom.InstanceNumber,
    ...
    def load_study_(
        ...
        for index, study in study_df.iterrows():
            ...
            path = os.path.join(
                ...
                study["SOPInstanceUID"] + extension,
            )
        ...
        data_point = {
            ...
            "thickness": dicom_object.thickness,
        }
    ...
    def load_studies(
        ...
        studies_colunm: Optional[str] = "StudyInstanceUID",
        extension: Optional[str] = ".dcm",
        save_to: str = "post_processed",
        maximum_pad: int = 824,
    ):
        """
        Load a study from the RSNA dataset.
        ...
        df = self.read_splits_(split=split, engine=engine)
        unique_studies_uid = self.get_unique_studies_(
            df=df, studies_colunm=studies_colunm
        )
        total_tudies = len(unique_studies_uid)
        post_processed_path = os.path.join(self.dataset_path, save_to, split + ".h5")
        ...
        for index, study_uid in tqdm(enumerate(unique_studies_uid), total=total_tudies):
            data_point = self.load_study_(
                split=split,
                df=df,
                study_uid=study_uid,
                extension=extension,
                maximum_pad=maximum_pad,
            )
            ...
            if index == 0:
                ...
                with h5py.File(post_processed_path, "a") as file:
                    for collect_item_name in collect_list:
                        ...
                        file[collect_item_name].resize(
                            (
                                file[collect_item_name].shape[0]
                                + collected_item.shape[0]
                            ),
                            axis=0,
                        )
                        file[collect_item_name][
                            -collected_item.shape[0] :
                        ] = collected_item
    ...
    def segment_study_(
        self, split: str, study_uid: str, series_uid: str, output_path: str
    ):
        ...
        if directory_not_exists(output_path):
            ...
            totalsegmentator(
                input=input_path, output=output_path, preview=True, task="lung_vessels"
            )
        else:
    ...
    def segment_studies(
        ...
        for index, study_uid in tqdm(
            enumerate(unique_studies_uid), total=total_studies
        ):
            series_uid = df[df[studies_colunm] == study_uid][
                "SeriesInstanceUID"
            ].to_list()[0]
    ...
    def convert_study_to_nifti(
        self, df, split: str, study_uid: str, series_uid: str, output_path: str
    ):
    ...
    def convert_studies_to_nifti(
        ...
        for index, study_uid in tqdm(
            ...
            output_path, split, study_uid, series_uid = self.convert_study_to_nifti(
                df=df,
                split=split,
                study_uid=study_uid,
                series_uid=series_uid,
    ...
    def crop_studies_from_masks(
        ...
        for index, study_uid in tqdm(
            ...
            output_path = Path(
                os.path.join(save_to, split, study_uid, series_uid, "volume.nii.gz")
            )
            ...
            try:
                self._crop_study_from_mask(
                    volume_path=nifti_path,
                    mask_roi_names=mask_roi_names,
    ...
    def _crop_study_from_mask(
        self, volume_path: str, mask_roi_names: list, mask_path: str, save_to: str
    ):

        ### Msks ###
        mask_volume = self.collect_volume_from_masks(
            mask_roi_names, mask_path
        )  # Collect volume from masks
        bbox = get_bbox_from_mask(
            mask=mask_volume, outside_value=0, addon=0
        )  # Creat bounding box from masks selected
        ...
        croped_volume_nifti = nib.Nifti1Image(
            croped_volume, affine=voume_nib.affine
        )  # convert numpy to nifit
    ...
    def simplfy_labels(
        ...
        for index, study_uid in tqdm(
            ...
            data_list.append(
                [split, study_uid, series_uid, label]
        ...
        df_result = pd.DataFrame(
            data_list, columns=["split", "study_uid", "series_uid", "label"]
        )
