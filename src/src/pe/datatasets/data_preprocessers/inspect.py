...
class InspectPreprocessing(DataPreprocessing):
    ...
    def process_files_parallel(
        self, max_workers: int = 10, start: int = 0, end: int = -1
    ):
    ...
    def segment_study_(self, input_nifti_path, outtput_nifti_path, device="gpu"):
        ...
        mask_path: os.PathLike = os.path.join(
            self.directory_path, "masks", input_nifti_path
        )
        output_path: os.PathLike = os.path.join(
            outtput_nifti_path, input_nifti_path.strip(".nii.gz")
        )
    ...
    def segment_studies(self, save_to: str, start: int = 0, end: int = -1):
        ...
        for nifti_path in tqdm(
            data_dir["nifti"], total=self.len_directory_contents // 2
        ):
            sucsses = self.segment_study_(
                input_nifti_path=nifti_path, outtput_nifti_path=save_to
            )
        ...
        with open("missing_nifti.csv", "w", newline="") as csv_file:
            ...
            csv_out.writerows(
                [missing_nifitis[index]] for index in range(0, len(missing_nifitis))
            )
