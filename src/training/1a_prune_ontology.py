...
def main():
    parser = argparse.ArgumentParser(
        description="Process and prune ontology based on dataset"
    )
    parser.add_argument(
        "--input-dataset", type=str, required=True, help="Path to input parquet dataset"
    )
    parser.add_argument(
        "--input-ontology",
        type=str,
    ...
    parser.add_argument(
        "--num-processors", type=int, default=32, help="Number of processors to use"
    )
    parser.add_argument(
        "--remove-ontologies",
        type=str,
