...
def run_analysis(
    ...
    if confidence_interval:
        auroc_train_ci, auroc_ci = bootstrap_auc(y_train, y_train_proba), bootstrap_auc(
            y_test, y_test_proba
        )
...
def set_up_motor_task(
    ...
    if "subset" not in label_csv:
        inspect_split_csv = os.path.join(
            TARGET_DIR, f"motor_model_{month_date_hour}", "main_split.csv"
        )
    else:
        inspect_split_csv = os.path.join(
            TARGET_DIR, f"motor_model_{month_date_hour}", "main_split_subset.csv"
        )
    ...
    if (not from_pretrained) and (not inference):
        ...
        tokenizer.save_pretrained(
            os.path.join(TARGET_DIR, f"motor_model_{month_date_hour}")
        )
    ...
    if not from_pretrained:
        ...
        with open(
            os.path.join(
                TARGET_DIR, f"motor_model_{month_date_hour}", "motor_task.pkl"
            ),
            "wb",
        ) as f:
...
def make_as_tensor(motor_batch):
    for key in motor_batch:
        if type(motor_batch[key]) == dict:
            motor_batch[key] = {
                k: torch.from_numpy(v) for k, v in motor_batch[key].items()
            }
            ...
            if "conv" in key or "downsample.0" in key:
                ...
                if len(weight_3d.shape) == 5:  # Only convert if it's a 3D conv weight
                    if "conv0.weight" in key:  # Initial convolution layer
                        ...
                        if (
                            weight_2d.shape[1] != new_state_dict[key].shape[1]
                            ...
                            if (
                                ...
                                weight_2d.shape[1] == 1
                                and new_state_dict[key].shape[1] == 3
                            ):
                ...
                if "conv" in modified_key or "downsample.0" in modified_key:
                    ...
                    if len(weight_3d.shape) == 5:
                        if "conv0.weight" in modified_key:
                            ...
                            if (
                                weight_2d.shape[1]
                                != new_state_dict[modified_key].shape[1]
                            ):  # Handle channel mismatch
                                if (
                                    weight_2d.shape[1] == 3
                                    and new_state_dict[modified_key].shape[1] == 1
                                ):
                                    weight_2d = weight_2d.mean(dim=1, keepdim=True)
                                elif (
                                    weight_2d.shape[1] == 1
                                    and new_state_dict[modified_key].shape[1] == 3
                                ):
                                    weight_2d = weight_2d.repeat(1, 3, 1, 1)
                    ...
                    "bn" in modified_key
                    or "downsample.1" in modified_key
                    or "norm" in modified_key
                ):
...
def load_different_model(model, pretrained_dict, ddp=False):
    ...
    filtered_dict = {
        k: v for k, v in pretrained_dict.items() if k in model.state_dict()
    }
    pretrain_unique_dict = {
        k: v for k, v in pretrained_dict.items() if k not in model.state_dict()
    }
    model_unique_dict = {
        k: v for k, v in model.state_dict().items() if k not in pretrained_dict
    }
    ...
    try:
        ...
        filtered_dict = {
            k: v for k, v in pretrained_dict.items() if "classification" not in k
        }
    return model
...
def load_different_model_i3resnet(model, pretrained_dict, ddp=False):
    ...
    if conv1_weight.shape[1] == 3 and model.conv1.weight.shape[1] == 1:
        print(
            f"Adjusting conv1 weights from shape {conv1_weight.shape} to {model.conv1.weight.shape}"
        )
    ...
    if conv1_weight.shape[2] == 3 and model.conv1.weight.shape[2] == 7:
        ...
        conv1_weight = F.interpolate(
            conv1_weight, size=(7, 7, 7), mode="trilinear", align_corners=False
        )
...
def get_final_batch(
    image_path, nii_folder, motor_task, dataset_split, index_split, ontology
):
    patient_id_ls = []
    ct_time_ls = []
    for img_path in image_path:
        ...
        ct_time = " ".join(
            img_path.replace(nii_folder + "/", "").replace(".nii.gz", "").split("_")[1:]
        )
    ...
    for i in range(1, len(final_batch_ls), 1):
        ...
        final_batch["is_event"] = torch.cat(
            [final_batch["is_event"], one_final_batch["is_event"]], dim=0
        )
        final_batch["log_time"] = torch.cat(
            [final_batch["log_time"], one_final_batch["log_time"]], dim=0
        )
        final_batch["time"] = torch.cat(
            [final_batch["time"], one_final_batch["time"]], dim=0
        )
...
def logistic_regression(
    ...
    if type(y_train) == list:
        ...
        if confidence_interval:
            auroc_train, auroc_val, auroc_train_ci, auroc_val_ci = run_analysis(
                "Logistic Regression",
                y_train,
                y_train_proba,
                y_val,
    ...
    # y_train is an array
        ...
        for idx, label_col in enumerate(column_ls):
            ...
            print(
                ...
                len(y_val.tolist()),
                "unique values and counts:",
                np.unique(y_val_task, return_counts=True),
            )
            ...
            tree_model_proba_save_path = os.path.join(
                model_save_path,
                f"tree_model_proba_{epoch}epoch_{model_choice}_{label_col}_{month_date}.pkl",
            )
            ...
            if confidence_interval:
                ...
                auroc_train_dict[f"auroc_train_tree_ci_{label_col}"] = (
                    auroc_train_tree_ci
                )
...
def get_split_df(image_paths, label_df, X, nii_folder, task_set):
    ...
    for image_path in image_paths.squeeze():
        ...
        patient_datetime = (
            image_path.replace(nii_folder, "")
            .replace("anon_nii_gz/anon_nii_gz", "")
            .replace(".nii.gz", "")
            .replace("/", "")
        )
    ...
    for i, patient_datetime in enumerate(patient_datetime_ls):
        try:
            ...
            row = label_df.loc[label_df["patient_datetime"] == patient_datetime][
                labels_columns
            ].iloc[0, :]
            row_ls.append(row)
        except:
            print(
                f"patient_datetime {patient_datetime} not found in label_df, probs negative tte readmission, PH"
            )
...
def survival_probe(
    ...
    label_df_train, X_train = get_split_df(
        image_paths_train, label_df, X_train, nii_folder, task_set
    )
    label_df_test, X_test = get_split_df(
        image_paths_test, label_df, X_test, nii_folder, task_set
    )
    label_df_val, X_val = get_split_df(
        image_paths_val, label_df, X_val, nii_folder, task_set
    )
    ...
    for task in task_set:
        y_train = (
            label_df_train[f"tte_{task}"].astype(np.int32).to_numpy(),
            label_df_train[f"is_censored_{task}"].astype(bool).to_numpy(),
        )
        y_test = (
            label_df_test[f"tte_{task}"].astype(np.int32).to_numpy(),
            label_df_test[f"is_censored_{task}"].astype(bool).to_numpy(),
        )
        y_val = (
            label_df_val[f"tte_{task}"].astype(np.int32).to_numpy(),
            label_df_val[f"is_censored_{task}"].astype(bool).to_numpy(),
        )
        ...
        print(
            f"batch_size= {batch_size}, num_nodes= {num_nodes}, dropout= {dropout}, lr= {lr}"
        )
        ...
        if confidence_interval:
            tdcs_test_dict[f"tdcs_test_ci_{task}"] = bootstrap_tdcs(
                y_test[0], ~y_test[1], hazards
            )
...
class EvalSurv:
    ...
    def __init__(
        ...
        assert (
            type(durations) == type(events) == np.ndarray
    ...
    def add_censor_est(self, censor_surv, steps="post"):
        ...
        if not isinstance(censor_surv, EvalSurv):
            censor_surv = self._constructor(
                censor_surv, self.durations, 1 - self.events, None, steps=steps
            )
        self.censor_surv = censor_surv
        return self
    ...
    def add_km_censor(self, steps="post"):
        ...
        surv = pd.DataFrame(
            np.repeat(km.values.reshape(-1, 1), len(self.durations), axis=1),
            index=km.index,
        )
    ...
    def censor_durations(self, val):
        if val is not None:
            assert (
                self.durations[self.events == 0] == val[self.events == 0]
            ).all(), (
                "Censored observations need same `durations` and `censor_durations`"
            )
            assert (
                self.durations[self.events == 1] <= val[self.events == 1]
            ).all(), "`durations` cannot be larger than `censor_durations`"
            if (self.durations == val).all():
                warnings.warn(
                    "`censor_durations` are equal to `durations`."
    ...
    def concordance_td(self, method="adj_antolini"):
        ...
        return concordance_td(
            self.durations, self.events, self.surv.values, self._duration_idx(), method
        )
    ...
    def brier_score(self, time_grid, max_weight=np.inf):
        ...
        if self.censor_surv is None:
            raise ValueError(
                """Need to add censor_surv to compute Brier score. Use 'add_censor_est'
    ...
    def nbll(self, time_grid, max_weight=np.inf):
        ...
        bll = ipcw.binomial_log_likelihood(
            time_grid,
            self.durations,
            self.events,
            self.surv.values,
    ...
    def integrated_brier_score(self, time_grid, max_weight=np.inf):
        ...
        return ipcw.integrated_brier_score(
            time_grid,
            self.durations,
            self.events,
            self.surv.values,
    ...
    def integrated_nbll(self, time_grid, max_weight=np.inf):
        ...
        ibll = ipcw.integrated_binomial_log_likelihood(
            time_grid,
            self.durations,
            self.events,
            self.surv.values,
    ...
    def brier_score_admin(self, time_grid):
        ...
        bs = admin.brier_score(
            time_grid,
            self.durations,
            self.censor_durations,
            self.events,
            self.surv.values,
            self.index_surv,
            True,
    ...
    def integrated_brier_score_admin(self, time_grid):
        ...
        ibs = admin.integrated_brier_score(
            time_grid,
            self.durations,
            self.censor_durations,
            self.events,
    ...
    def nbll_admin(self, time_grid):
        ...
        bll = admin.binomial_log_likelihood(
            time_grid,
            self.durations,
            self.censor_durations,
            self.events,
    ...
    def integrated_nbll_admin(self, time_grid):
        ...
        ibll = admin.integrated_binomial_log_likelihood(
            time_grid,
            self.durations,
            self.censor_durations,
            self.events,
...
def test_c_statistic(times, is_censor, hazards):
    ...
    for time in sorted(list(set(times))):
        mask = (
            ((times > time) & (is_censor != 0)) | ((times >= time) & (is_censor == 0))
        ).astype(bool)
