...
warnings.filterwarnings(
    "ignore", message="The max_iter was reached which means the coef_ did not converge"
)
...
def train(
    ...
    config_dict["loadmodel_path"] = (
        loadmodel_path.split("/")[-1] if loadmodel_path else None
    )
    config_dict["learning_rate"] = learning_rate
    ...
    if use_crop:
        ...
        assert (
            "crop_lung" in nii_folder
        ), "Please use lung_crop folder for cropping experiments"
    else:
    ...
    if False:
        ...
        if True:
            ...
            label_df_tte = pd.read_csv(
                label_csv.replace("anon", "anon_tte"), dtype=data_types
            )
            ...
            for idx, row in label_df.iterrows():
                for label_col in label_column:
                    ...
                    if label_col == "same_visit_8192":
                        try:
                            labels[label_col].append(
                                CT_8192labels[row["patient_datetime"]]
                            )
                        except:
            ...
            for label_col in label_column:
                ...
                print(
                    label_col,
                    np.unique(labels_train[label_col], return_counts=True),
                    "labels_train",
                )
            if "same_visit_8192" in label_column:
            ...
            for label_col in label_column:
                ...
                print(
                    label_col,
                    np.unique(labels_valid[label_col], return_counts=True),
                    "labels_valid",
                )
            if "same_visit_8192" in label_column:
            ...
            if use_cachedataset:
                ...
                for i in range(len(image_paths_train)):
                    one_entry = {
                        "image": image_paths_train[i],
                        "label": list(labels_train_df.iloc[i].values.tolist()),
                        "image_path": image_paths_train[i],
                    }
                    data_train.append(one_entry)
                data_val = []
                for i in range(len(image_paths_valid)):
                    one_entry = {
                        "image": image_paths_valid[i],
                        "label": list(labels_valid_df.iloc[i].values.tolist()),
                        "image_path": image_paths_valid[i],
                    }
        ...
        if inference:
            ...
            for label_col in label_column:
                ...
                print(
                    label_col,
                    np.unique(labels_test[label_col], return_counts=True),
                    "labels_test",
                )
            if "same_visit_8192" in label_column:
            ...
            if True:
                if use_cachedataset:
                    ...
                    for i in range(len(image_paths_test)):
                        one_entry = {
                            "image": image_paths_test[i],
                            "label": list(labels_test_df.iloc[i].values.tolist()),
                            "image_path": image_paths_test[i],
                        }
        ...
        for idx, row in tqdm(label_df.iterrows(), total=len(label_df)):
            ...
            image_paths.append(
                nii_folder + "/" + str(row["study_series"]) + "/" + str(row["nii_name"])
            )
        ...
        if use_cachedataset:
            ...
            for i in range(len(image_paths_train)):
                one_entry = {
                    "image": str(image_paths_train[i]),
                    "label": str(labels_train[i]),
                }  # , 'image_path': [str(image_paths_train[i])]}
                data_train.append(one_entry)
            data_val = []
            for i in range(len(image_paths_valid)):
                one_entry = {
                    "image": str(image_paths_valid[i]),
                    "label": str(labels_valid[i]),
                }  # , 'image_path': [str(image_paths_valid[i])]}
                data_val.append(one_entry)
            data_test = []
            for i in range(len(image_paths_test)):
                one_entry = {
                    "image": str(image_paths_test[i]),
                    "label": str(labels_test[i]),
                }  # , 'image_path': [str(image_paths_test[i])]}
                data_test.append(one_entry)
        ...
        for idx, row in tqdm(label_df.iterrows(), total=len(label_df)):
            ...
            image_paths.append(nii_folder + "/" + str(row["Path"]))
    ...
    if model_choice == "unet" or model_choice == "unet_mtl":
        ...
        if not multitask:
            ...
            if "same_visit_8192" in label_column:
                ...
                num_classes = len(label_column)
        print(
            f"Building model imagenet_stage1_stage2_mtl_seg_resnet_100 with {num_classes} classes"
        )

        model = resnet152(n_input_channels=1)
        checkpoint = torch.load(
            pretrained_path_resnet, map_location=f"cuda:{torch.cuda.current_device()}"
        )
        checkpoint_updated = {}
        ...
        filtered_checkpoint = {
            k: v
            for k, v in checkpoint_updated.items()
    ...
    if not inference and ("tte" in model_choice):
        ...
        for epoch in range(starting_epoch + 1, starting_epoch + max_epochs):
            print("-" * 10)
            print(
                f"Current epoch {epoch} to train to epoch {starting_epoch + max_epochs -1}"
            )
            model.train()
            epoch_loss = 0
            epoch_loss_valid = 0
            ...
            # while True:
            for batch_data_idx in tqdm(
                range(len(train_loader)), total=len(train_loader), ncols=75
            ):
                step += 1
                try:
                ...
                if use_cachedataset:
                    if "image_path" in batch_data:
                        inputs, labels, image_path = (
                            batch_data["image"].to(device),
                            batch_data["label"],
                            batch_data["image_path"],
                        )
                    else:
                        (
                            inputs,
                ...
                patient_id = int(
                    image_path[0].replace(nii_folder + "/", "").split("_")[0]
                )
                ct_time = " ".join(
                    image_path[0]
                    .replace(nii_folder + "/", "")
                ...
                if mixed_precision:
                    ...
                    with autocast():
                        loss, _, time_independent_features = model(
                            inputs, final_batch, return_logits=False
                        )
            ...
            if (not prop_train) and (not prop_valid):
                print(
                    ...
                    os.path.join(
                        model_save_path,
                        f"best_metric_model_{epoch}epoch_{model_choice}_{month_date}.pth",
                    ),
                )
                torch.save(
                    model.state_dict(),
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                for val_data in tqdm(val_loader, ncols=75):
                    ...
                    if not use_cachedataset:
                        ...
                        if "image_path" in val_data:
                            val_images, val_labels, image_path = (
                ...
                metric_values, _, _, auroc_val, auroc_train_dict, auroc_val_dict = (
                    logistic_regression(
                        label_column,
                        X_train,
                        y_train,
                ...
                if auroc_val > best_metric and (not prop_train) and (not prop_valid):
                    ...
                    print(
                        ...
                        + os.path.join(
                            model_save_path,
                            f"best_metric_model_{epoch}epoch_{model_choice}.pth",
                        )
                    )
                    torch.save(
                        model.state_dict(),
                ...
                if not prop_train:
                    with open(
                        os.path.join(
                            model_save_path.replace("model_checkpoints", "features"),
                            f"X_train_{epoch}epoch_{model_choice}.pkl",
                        ),
                        "wb",
                    ) as f:
                    ...
                    with open(
                        os.path.join(
                            model_save_path.replace("model_checkpoints", "features"),
                            f"image_paths_train_{epoch}epoch_{model_choice}.pkl",
                        ),
                        "wb",
                    ) as f:
                    ...
                    with open(
                        os.path.join(
                            model_save_path.replace("model_checkpoints", "features"),
                            f"y_train_{epoch}epoch_{model_choice}.pkl",
                        ),
                        "wb",
                    ) as f:
                ...
                if not prop_valid:
                    with open(
                        os.path.join(
                            model_save_path.replace("model_checkpoints", "features"),
                            f"X_val_{epoch}epoch_{model_choice}.pkl",
                        ),
                        "wb",
                    ) as f:
                        pickle.dump(X_val, f)
                    with open(
                        os.path.join(
                            model_save_path.replace("model_checkpoints", "features"),
                            f"image_paths_val_{epoch}epoch_{model_choice}.pkl",
                        ),
                        "wb",
                    ) as f:
                    ...
                    with open(
                        os.path.join(
                            model_save_path.replace("model_checkpoints", "features"),
                            f"y_val_{epoch}epoch_{model_choice}.pkl",
                        ),
                        "wb",
                    ) as f:
                        pickle.dump(y_val, f)
        ...
        for epoch in range(starting_epoch + 1, starting_epoch + max_epochs):
            ...
            for batch_data in tqdm(train_loader, ncols=75):
                ...
                if use_cachedataset:
                    ...
                    if "same_visit_8192" not in label_column:
                        try:
                            labels = [
                                -1 if label[0] == "Censored" else int(label[0])
                                for label in labels
                            ]
                        except:
                            labels = convert_to_list(labels)
                            labels = [
                                -1 if label == "Censored" else int(label)
                                for label in labels
                            ]
                ...
                if False:
                    ...
                    with autocast():
                        ...
                        outputs, train_features = model(
                            inputs, return_features=True, multitask=multitask
                        )
                    # Backward pass and optimization are handled with GradScaler
                    if "same_visit_8192" in label_column:
                        if type(labels) == list:
                            labels = torch.tensor(
                                labels, device=outputs.device, dtype=outputs.dtype
                            )
                            print(type(labels), labels)
                        try:
                            # ddp
                            loss = loss_function(
                                outputs.squeeze(), labels.squeeze().float()
                            )
                        except:
                            # single gpu
                            loss = loss_function(
                                torch.tensor(outputs, device=device),
                                torch.tensor(labels, device=device).seqeeze().float(),
                            )
                    else:
                        loss = loss_function(
                            outputs[0].squeeze(), labels[0].squeeze().float()
                        )
                        for i in range(1, labels.shape[1]):
                            if labels[i].item() == -1:
                                ...
                                mask = torch.tensor(
                                    [1.0], device=loss.device, dtype=loss.dtype
                                )
                            loss += (
                                loss_function(outputs[i].squeeze(), labels[i].squeeze())
                                * mask
                            )
                    print(loss, "loss -----------------------")
                    scaler.scale(loss).backward()
                ...
                if True:
                    ...
                    # normal MTL, not same_visit_8192
                        ...
                        loss = torch.tensor(
                            0.0, device=outputs[0].device, dtype=outputs[0].dtype
                        )
                        for i in range(0, labels.shape[1]):
                            ...
                            loss += (
                                loss_function(
                                    outputs[i].squeeze(), labels[i].squeeze().float()
                                )
                                * mask.squeeze()
                            )
                ...
                image_paths_train.append(image_path)
                y_train.append(labels)
                y_proba_train.append(
                    [output.cpu().detach().numpy() for output in outputs]
                )
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                for val_data in tqdm(val_loader, ncols=75):
                    ...
                    if not use_cachedataset:
                        ...
                        val_labels = torch.from_numpy(
                            np.array(val_labels, dtype=int)
                        ).to(device)
                        val_labels = val_labels.unsqueeze(1)
                    with torch.no_grad():
                        outputs, val_features = model(
                            val_images, return_features=True, multitask=multitask
                        )
                        ...
                        if "same_visit_8192" in label_column:
                            ...
                            try:
                                loss = loss_function(
                                    outputs.squeeze(), val_labels.squeeze().float()
                                )
                            except:
                                outputs_ = [o.cpu() for o in outputs]
                                loss = loss_function(
                                    torch.tensor(np.array(outputs_), device=device),
                                    torch.tensor(np.array(val_labels), device=device)
                                    .seqeeze()
                                    .float(),
                                )
                        else:
                ...
                try:
                    ...
                    image_paths_val = np.concatenate(
                        np.array(image_paths_val).reshape(-1, 1), axis=0
                    )
                y_val = np.concatenate(y_val, axis=1).transpose()
                X_train = np.concatenate(X_train, axis=0)
                try:
                    ...
                    image_paths_train = np.concatenate(
                        np.array(image_paths_train).reshape(-1, 1), axis=0
                    )
                y_train = np.concatenate(y_train, axis=1).transpose()
                ...
                # AUROC
                y_proba_train = (
                    np.concatenate(y_proba_train, axis=1)
                    .transpose()
                    .squeeze()
                    .reshape(-1, len(label_column))
                )
                y_proba_val = (
                    np.concatenate(y_proba_val, axis=1)
                    .transpose()
                    .squeeze()
                    .reshape(-1, len(label_column))
                )
                for idx, task in enumerate(label_column):
                    ...
                    try:
                        idx_uncensored = np.where(
                            (y_train != "Censored") & (y_train != -1)
                        )[0]
                        auroc_train = sklearn.metrics.roc_auc_score(
                            y_train[idx_uncensored, idx],
                            y_proba_train[idx_uncensored, idx],
                        )

                        idx_uncensored_val = np.where(
                            (y_val != "Censored") & (y_val != -1)
                        )[0]
                        auroc_val = sklearn.metrics.roc_auc_score(
                            y_val[idx_uncensored_val, idx],
                            y_proba_val[idx_uncensored_val, idx],
                        )
                        print(
                            f"=====NN model output: Train {task} AUROC: {auroc_train:.4f}, Val {task} AUROC: {auroc_val:.4f}"
                        )
                    except:
                ...
                if not prop_train:
                    ...
                    with open(
                        os.path.join(
                            model_save_path.replace("model_checkpoints", "features"),
                            f"y_proba_train_{epoch}epoch_{model_choice}.pkl",
                        ),
                        "wb",
                    ) as f:
                ...
                if not prop_valid:
                    ...
                    with open(
                        os.path.join(
                            model_save_path.replace("model_checkpoints", "features"),
                            f"y_proba_val_{epoch}epoch_{model_choice}.pkl",
                        ),
                        "wb",
                    ) as f:
        ...
        for epoch in range(starting_epoch + 1, starting_epoch + max_epochs):
            ...
            while True:
                try:
                    ...
                    if step % 100 == 0:
                        print(
                            f"Current step: {step}, time used: {datetime.now() - START_TIME}"
                        )
                    if use_cachedataset:
                        if "image_path" in batch_data:
                            ...
                            image_path = image_paths_train_[step]
                        try:
                            labels = torch.from_numpy(np.array(labels, dtype=int)).to(
                                device
                            )
                    ...
                    # print(f"{step}/{epoch_len}, train_loss: {loss.item():.4f}, time_used: {time_used}", end="  ")
                    writer.add_scalar(
                        "train_loss", loss.item(), epoch_len * (epoch - 1) + step
                    )
                    # run.track(loss.item(), name='train_loss', step=epoch_len * (epoch-1) + step, context={'subset': 'train'})
                except StopIteration:
            ...
            print(
                "y_train after:", y_train.shape, np.unique(y_train, return_counts=True)
            )
            for param_group in optimizer.param_groups:
                current_lr = param_group["lr"]
            # run.track(current_lr, name='current_lr', step=epoch, context={'subset': 'train'})
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                for val_data in tqdm(val_loader, ncols=75):
                    if not use_cachedataset:
                        ...
                        try:
                            ...
                            continue
                    with torch.no_grad():
                        val_outputs, val_features = model(
                            val_images, return_features=True
                        )

                        if (
                            val_features.shape[0] == batch_size
                            and val_labels.shape[0] == batch_size
                        ):
                        ...
                        loss_valid = loss_function(
                            val_outputs.squeeze(), val_labels.squeeze()
                        )
                ...
                for key, value in auroc_val_dict.items():
                    ...
                    writer.add_scalar(key, value, epoch)
                print(
                    "censor_count valid:", censor_count_valid, "out of", len(val_loader)
                )
                ...
                if metric > best_metric:
                    ...
                    print(
                        ...
                        os.path.join(
                            model_save_path,
                            f"best_metric_model_{epoch}epoch_{model_choice}_{finetune_label}{'_lb' if linear_probe else ''}.pth",
                        ),
                    )
                    torch.save(
                        model.state_dict(),
    ...
    # inference only
        ...
        if generate_train_features:
            ...
            if not prop_train:
                with open(
                    os.path.join(
                        model_save_path.replace("model_checkpoints", "features"),
                        f"X_train_{starting_epoch}epoch_{model_choice}.pkl",
                    ),
                    "wb",
                ) as f:
                    pickle.dump(X_train, f)
                with open(
                    os.path.join(
                        model_save_path.replace("model_checkpoints", "features"),
                        f"image_paths_train_{starting_epoch}epoch_{model_choice}.pkl",
                    ),
                    "wb",
                ) as f:
                    pickle.dump(image_paths_train, f)
                with open(
                    os.path.join(
                        model_save_path.replace("model_checkpoints", "features"),
                        f"y_train_{starting_epoch}epoch_{model_choice}.pkl",
                    ),
                    "wb",
                ) as f:
                    pickle.dump(y_train, f)
            ...
            # while True:
            for batch_data_idx in tqdm(
                range(len(val_loader)), total=len(val_loader), ncols=75
            ):
                step_valid += 1
                try:
            ...
            if not prop_valid:
                with open(
                    os.path.join(
                        model_save_path.replace("model_checkpoints", "features"),
                        f"X_val_{starting_epoch}epoch_{model_choice}.pkl",
                    ),
                    "wb",
                ) as f:
                    pickle.dump(X_val, f)
                with open(
                    os.path.join(
                        model_save_path.replace("model_checkpoints", "features"),
                        f"image_paths_val_{starting_epoch}epoch_{model_choice}.pkl",
                    ),
                    "wb",
                ) as f:
                    pickle.dump(image_paths_val, f)
                with open(
                    os.path.join(
                        model_save_path.replace("model_checkpoints", "features"),
                        f"y_val_{starting_epoch}epoch_{model_choice}.pkl",
                    ),
                    "wb",
                ) as f:
                    pickle.dump(y_val, f)
        ...
        # train/val feature from disk
            ...
            with open(
                os.path.join(
                    model_save_path.replace("model_checkpoints", "features"),
                    f"X_train_{starting_epoch}epoch_{loadmodel_feature}.pkl",
                ),
                "rb",
            ) as f:
            ...
            with open(
                os.path.join(
                    model_save_path.replace("model_checkpoints", "features"),
                    f"image_paths_train_{starting_epoch}epoch_{loadmodel_feature}.pkl",
                ),
                "rb",
            ) as f:
            ...
            with open(
                os.path.join(
                    model_save_path.replace("model_checkpoints", "features"),
                    f"y_train_{starting_epoch}epoch_{loadmodel_feature}.pkl",
                ),
                "rb",
            ) as f:
            ...
            with open(
                os.path.join(
                    model_save_path.replace("model_checkpoints", "features"),
                    f"X_val_{starting_epoch}epoch_{loadmodel_feature}.pkl",
                ),
                "rb",
            ) as f:
                X_val = pickle.load(f)
            with open(
                os.path.join(
                    model_save_path.replace("model_checkpoints", "features"),
                    f"image_paths_val_{starting_epoch}epoch_{loadmodel_feature}.pkl",
                ),
                "rb",
            ) as f:
            ...
            with open(
                os.path.join(
                    model_save_path.replace("model_checkpoints", "features"),
                    f"y_val_{starting_epoch}epoch_{loadmodel_feature}.pkl",
                ),
                "rb",
            ) as f:
                y_val = pickle.load(f)
        ...
        if False:
            ...
            linear_model_save_path = os.path.join(
                model_save_path,
                f"linear_model_{starting_epoch}epoch_{model_choice}_{month_date}.pkl",
            )
        ...
        X_test_path = os.path.join(
            model_save_path.replace("model_checkpoints", "features"),
            f"X_test_{starting_epoch}epoch_{model_choice}.pkl",
        )
        image_paths_test_path = os.path.join(
            model_save_path.replace("model_checkpoints", "features"),
            f"image_paths_test_{starting_epoch}epoch_{model_choice}.pkl",
        )
        y_test_path = os.path.join(
            model_save_path.replace("model_checkpoints", "features"),
            f"y_test_{starting_epoch}epoch_{model_choice}.pkl",
        )

        # calculate test features, if not in disk
        if (
            not os.path.exists(X_test_path)
            or not os.path.exists(image_paths_test_path)
            or not os.path.exists(y_test_path)
        ):
            X_test = []
            image_paths_test = []
            y_test = []
            ...
            print(
                "================",
                f"{'val_loader' if test_loader == val_loader else 'test_loader'}",
                "for testing len:",
                len(test_loader),
                "----------",
            )
            ...
            while True:
                try:
                    ...
                    if step_test % 100 == 0:
                        print(
                            f"Current step: {step_test}, time used: {datetime.now() - START_TIME}"
                        )
                    ...
                    with torch.no_grad():
                        try:
                            y_pred, time_independent_features = model(
                                inputs,
                                return_features=True,
                                inference=inference,
                                return_logits=True,
                    ...
                    if "RSNA" not in dataformat and "chexpert" not in dataformat:
                        ...
                        final_batch, motor_batch = get_final_batch(
                            image_path,
                            nii_folder,
                            motor_task,
                            test_dataset,
            ...
            if "RSNA" not in dataformat:
                ...
                with open(
                    os.path.join(
                        model_save_path.replace("model_checkpoints", "features"),
                        f"time_dependent_logits_ls_{starting_epoch}epoch_{model_choice}.pkl",
                    ),
                    "wb",
                ) as f:
                ...
                with open(
                    os.path.join(
                        model_save_path.replace("model_checkpoints", "features"),
                        f"is_event_ls_{starting_epoch}epoch_{model_choice}.pkl",
                    ),
                    "wb",
                ) as f:
                ...
                with open(
                    os.path.join(
                        model_save_path.replace("model_checkpoints", "features"),
                        f"log_time_ls_{starting_epoch}epoch_{model_choice}.pkl",
                    ),
                    "wb",
                ) as f:
                ...
                with open(
                    os.path.join(
                        model_save_path.replace("model_checkpoints", "features"),
                        f"time_ls_{starting_epoch}epoch_{model_choice}.pkl",
                    ),
                    "wb",
                ) as f:
        ...
        if False:
            ...
            if label_column:
                if "chexpert" in dataformat:
                    ...
                    image_paths_train = np.concatenate(
                        (image_paths_train, image_paths_val), axis=0
                    )
                    y_train = np.concatenate((y_train, y_val), axis=0)
                    X_test = np.concatenate((X_test, X_val), axis=0)
                    image_paths_test = np.concatenate(
                        (image_paths_test, image_paths_val), axis=0
                    )
            ...
            if survival_tasks:
                tdcs_train_dict, tdcs_test_dict, ibs_train_dict, ibs_test_dict = (
                    survival_probe(
                        survival_tasks,
                        X_train,
                        image_paths_train,
