...
def main(args):
    ...
    train_transforms = Compose(
        [ScaleIntensity(), Resize((224, 224, 200)), RandRotate90()]
    )
    ...
    train_loader = DataLoader(
        Subset(loader.dataset, train_indices), batch_size=1, shuffle=True
    )
    test_loader = DataLoader(
        Subset(loader.dataset, test_indices), batch_size=1, shuffle=False
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--csv_file", type=str, default="//data/RSNAPE/simplified_labels/train.csv"
    )
