...
def logistic_regression(column_ls, X_train, y_train, X_val, y_val, model_save_path, model_choice, epoch, month_date, metric_values, run, writer, tune_linearprobe):
    ...
    if type(y_train) == list:
        ...
        auroc_train, auroc_val = run_analysis("Logistic Regression", y_train, y_train_proba, y_val, y_val_proba)
    ...
    # y_train is an array
        ...
        for idx, label_col in enumerate(column_ls):
            ...
            print(f'num of censored for {label_col} in train', len(y_train.tolist())-len(y_train_list), 'out of', len(y_train.tolist()))
            ...
            print(f'num of censored for {label_col} in val', len(y_val.tolist())-len(y_val_task), 'out of', len(y_val.tolist()))
            ...
            auroc_train, auroc_val = run_analysis("Logistic Regression", y_train_list, y_train_proba, y_val_task, y_val_proba, label_col)
            auroc_train_tree, auroc_val_tree = run_analysis("XGBoost", y_train_list, y_train_proba_tree, y_val_task, y_val_proba_tree, label_col)
