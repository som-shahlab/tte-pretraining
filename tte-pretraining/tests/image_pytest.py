...
def load_real_data():
    ...
    train_transforms = Compose([ScaleIntensity(), EnsureChannelFirst(), Resize((224, 224, 200))])

    dataset = ImageDataset(image_files=images, labels=torch.nn.functional.one_hot(torch.as_tensor(labels)).float(), transform=train_transforms)
...
def setup_model():
    ...
    state_dict = torch.load(loadmodel_path, map_location=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))

    model = DenseNet121(spatial_dims=3, in_channels=1, out_channels=2).to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
...
def test_labels_and_features_matching(load_real_data, setup_model):
    ...
    features_train = np.array(features_train).squeeze()  # Convert list of features to array
    labels_train = np.array(labels_train)

    # Check if the number of features matches the number of labels
    assert features_train.shape[0] == labels_train.shape[0], "Number of features should match the number of labels"
...
def test_model_loading(setup_model):
    ...
    assert 'classification.out.weight' in state_dict or 'features.conv0.weight' in state_dict, "Model does not have the expected layer weights"
...
def test_real_data_loading(load_real_data):
    ...
    for i in range(len(dataset)):
        image, label = dataset[i]
        assert image.shape == (1, 224, 224, 200), f"Image shape should be (1, 224, 224, 200), but got {image.shape}"
