...
model = DenseNet121_TTE(spatial_dims=3, in_channels=1, out_channels=2, time_bins=motor_task.time_bins, pretraining_task_info=motor_task.get_task_config().task_kwargs['pretraining_task_info'], final_layer_size=motor_task.final_layer_size, vocab_size=tokenizer.vocab_size, device=device).to(device)
...
val_loader = DataLoader(
    val_ds, batch_size=1, num_workers=4, pin_memory=pin_memory
)
...
test_loader = DataLoader(
    test_ds, batch_size=1, num_workers=4, pin_memory=pin_memory
)
